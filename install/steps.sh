#!/usr/bin/env bash
# Shared install logic: state detection, the steps, and the manual-work report.
# Sourced by both installers, never executed.
#
# There are two front ends over this file and they differ only in FLOW:
#
#   install.sh          a dashboard and a menu. Shows everything at once, you pick.
#                       Better once the box is partly built and you want one thing.
#   install-guided.sh   a linear walkthrough. Explains each step before running it,
#                       with progress and failure recovery. Better on a fresh box.
#
# Both call the functions below, so they cannot drift apart. Picking one is deleting the
# other file, and nothing else.
